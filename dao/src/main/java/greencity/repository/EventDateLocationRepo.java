...
public interface EventDateLocationRepo extends JpaRepository<EventDateLocation, Long>,
    ...
    boolean hasLiveOccurrence(@Param("eventId") Long eventId, @Param("now") LocalDateTime now);

    @Query("SELECT MIN(dl.startDate) FROM EventDateLocation dl "
        + "WHERE dl.event.id = :eventId AND dl.startDate > :now")
    LocalDateTime findEarliestFutureStartDate(@Param("eventId") Long eventId, @Param("now") LocalDateTime now);

    @Query("SELECT MAX(dl.finishDate) FROM EventDateLocation dl WHERE dl.event.id = :eventId")
    LocalDateTime findLatestFinishDate(@Param("eventId") Long eventId);

    @Query("SELECT dl FROM EventDateLocation dl "
        + "WHERE dl.event.id = :eventId AND dl.startDate > :now "
        + "ORDER BY dl.startDate ASC")
    List<EventDateLocation> findEarliestFutureOccurrence(@Param("eventId") Long eventId,
    ...
    @Query("SELECT dl FROM EventDateLocation dl "
        ...
        + "AND :now >= dl.startDate AND :now <= dl.finishDate "
        + "ORDER BY dl.startDate ASC")
    List<EventDateLocation> findLiveOccurrence(@Param("eventId") Long eventId, @Param("now") LocalDateTime now);
}